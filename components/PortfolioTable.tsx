      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-neutral-200 bg-neutral-50/70">
            ...
            <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Entry Stage</th>
            <th className="text-right px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Invested to Date</th>
            <th className="text-right px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Ownership</th>
            <th className="text-right px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Planned Reserves</th>
            <th className="text-right px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Deployed Reserves</th>
            <th className="text-right px-5 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Initial Investment</th>
          </tr>
        </thead>
        <tbody>
          {companies.map((co) => {
            ...
            return (
              <Fragment key={co.id}>
                <tr
                  className={`border-t border-slate-50 transition-colors ${isExpanded ? 'bg-neutral-50/80' : 'hover:bg-neutral-50/60'}`}
                >
                  {/* Expand toggle */}
                  ...
                  <td className="px-4 py-3 text-neutral-600 text-xs">{co.entry_stage ?? <span className="text-slate-300">—</span>}</td>
                  ...
                  <td className="px-4 py-3 text-right text-neutral-700">{co.plannedReserves > 0 ? fmt$$(co.plannedReserves) : <span className="text-slate-300">—</span>}</td>
                  <td className="px-4 py-3 text-right text-neutral-700">{co.deployedReserves > 0 ? fmt$$(co.deployedReserves) : <span className="text-slate-300">—</span>}</td>
                  <td className="px-5 py-3 text-right text-neutral-700">{co.initialInvestment > 0 ? fmt$$(co.initialInvestment) : <span className="text-slate-300">—</span>}</td>
                </tr>
                            ...
                            {timeline.map((item, i) => {
                              ...
                              return (
                                <tr key={`safe-${safe.id}`} className="hover:bg-white/60 transition-colors">
                                  ...
                                  <td className="px-4 py-2 text-neutral-600">{fmtDate(safe.date)}</td>
                                  <td className="px-4 py-2">
