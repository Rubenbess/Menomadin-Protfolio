        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            {(['all', 'impact', 'venture'] as const).map(s => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                  strategy === s ? 'bg-primary-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border transition-all ${
              onlyWithData ? 'bg-primary-500 text-white border-transparent' : 'bg-white text-slate-500 border-slate-200 hover:text-slate-700'
            }`}
          >
                    ...
                    <tr key={co.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      {kpi ? (
                        ...
                        <td colSpan={8} className="px-4 py-3.5 text-center text-xs text-slate-300">
                          No KPI data —{' '}
                          <Link href={`/companies/${co.id}`} className="text-primary-300 hover:text-primary-500 hover:underline">
