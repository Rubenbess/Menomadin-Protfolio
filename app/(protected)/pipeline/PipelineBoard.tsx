...
const COLOR_HEADER: Record<string, string> = {
  ...
  indigo: 'bg-primary-100 text-primary-600 border-violet-200',
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
            ...
            {askLabel && (
              <span className="text-xs bg-primary-50 text-primary-500 rounded-lg px-2 py-0.5">{askLabel}</span>
            )}
          </div>
  ...
  return (
    <div className="animate-fade-in">
      <div className="page-header">
        ...
        <div className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-xl p-1 shadow-sm">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${view === 'board' ? 'bg-primary-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            <Plus size={12} /> Board
          </button>
          <button
            onClick={() => setView('analytics')}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${view === 'analytics' ? 'bg-primary-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <div className="flex gap-4 overflow-x-auto pb-6 pt-1 px-1 -mx-1">
          ...
          <div className="flex-shrink-0 w-64">
            <button
              ...
              className="w-full h-12 flex items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-200 text-sm text-slate-400 hover:border-primary-200 hover:text-primary-500 transition-colors"
            >
        ...
        <DragOverlay>
          {activeEntry && (
            <div className="bg-white rounded-xl border border-primary-200 shadow-xl p-3 w-64 rotate-2 opacity-95">
