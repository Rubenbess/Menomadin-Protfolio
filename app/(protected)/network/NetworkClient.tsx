  ...
  if (co.logo_url) {
    return (
      <img
        src={co.logo_url}
        alt={co.name}
        className="w-5 h-5 rounded-md object-contain bg-slate-50 ring-1 ring-slate-200 flex-shrink-0"
      />
    )
  }
  return (
    <div className="w-5 h-5 rounded-md bg-violet-100 flex items-center justify-center flex-shrink-0">
      <span className="text-[9px] font-bold text-violet-600">{co.name[0]}</span>
    </div>
  )
}
  ...
  return (
    <div
      ...
      className={`bg-white rounded-2xl p-5 cursor-pointer transition-all ring-1 ${
        selected
          ? 'ring-violet-400 shadow-lg shadow-violet-100'
          : isHighlighted
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        ...
        <span className="flex-shrink-0 text-xs font-semibold bg-violet-100 text-violet-700 rounded-full px-2.5 py-0.5">
      ...
      <div className="flex flex-col gap-1.5">
        {node.companies.slice(0, 5).map(co => (
          <Link
            ...
            className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl bg-slate-50 hover:bg-violet-50 hover:text-violet-700 group/chip transition-colors"
          >
            <Avatar co={co} />
            <span className="text-xs font-medium text-slate-700 group-hover/chip:text-violet-700 transition-colors truncate">{co.name}</span>
        ...
        {node.companies.length > 5 && (
          <p className="text-xs text-slate-400 px-2.5">+{node.companies.length - 5} more</p>
        )}
      </div>
    </div>
  )
...
export default function NetworkClient({ nodes, allCompanies }: Props) {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={s.label} className="bg-white rounded-xl border border-slate-200 px-4 py-3 shadow-sm">
            ...
            <p className="text-base font-bold text-slate-900 truncate" title={s.value}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-5">
        ...
        {selectedInvestor && (
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-800 px-2.5 py-1.5 rounded-lg hover:bg-slate-100 transition-colors"
          >
      ...
      {selectedNode && (
        <div className="mb-5 bg-violet-50 rounded-2xl border border-violet-200 px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold text-violet-500 uppercase tracking-wider mb-0.5">Selected co-investor</p>
            ...
            <button
              onClick={() => setSelectedInvestor(null)}
              className="p-1.5 text-violet-400 hover:text-violet-700 hover:bg-violet-100 rounded-lg transition-colors flex-shrink-0"
            >
              <X size={14} />
            </button>
          </div>
          ...
          {siblingInvestors.size > 0 && (
            <div className="mt-3 pt-3 border-t border-violet-200">
              <p className="text-xs text-violet-500 font-medium mb-2">Co-appears with:</p>
              <div className="flex flex-wrap gap-1.5">
                {[...siblingInvestors].map(inv => (
                  <button
                    key={inv}
                    onClick={() => setSelectedInvestor(inv)}
                    className="text-xs px-2.5 py-1 rounded-full bg-violet-100 text-violet-700 hover:bg-violet-200 transition-colors font-medium"
                  >
                    {inv}
                  </button>
                ))}
          ...
          <div className="flex flex-wrap gap-2">
            {companiesWithout.map(co => (
              <Link
                key={co.id}
                href={`/companies/${co.id}`}
                className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-xl border border-slate-200 text-xs text-slate-500 hover:text-violet-600 hover:border-violet-200 transition-colors"
              >
                <Avatar co={co} />
                {co.name}
              </Link>
