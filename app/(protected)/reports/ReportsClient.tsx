    ...
    <div className="bg-white rounded-2xl shadow-card ring-1 ring-black/[0.04] p-5">
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={() => handle('pdf')}
          disabled={!!loading}
          className="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-semibold border border-violet-200 text-primary-600 bg-primary-50 hover:bg-primary-100 transition-all disabled:opacity-50"
        >
          {pdfDone
            ? <><CheckCircle2 size={13} /> Downloaded</>
            : loading === 'pdf'
...
function DownloadButton({ label, onClick, color }: { label: string; onClick: () => Promise<void>; color: 'white' | 'violet' }) {
  ...
  const cls  = color === 'white'
    ? `${base} bg-white text-primary-600 hover:bg-primary-50`
    : `${base} bg-primary-500 text-white hover:bg-primary-300 ring-1 ring-white/20`
