...
function Avatar({ co }: { co: CompanySnippet }) {
  ...
  return (
    <div className="w-5 h-5 rounded-md bg-gold-100 flex items-center justify-center flex-shrink-0">
      <span className="text-[9px] font-bold text-gold-500">{co.name[0]}</span>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        ...
        <span className="flex-shrink-0 text-xs font-semibold bg-gold-100 text-gold-600 rounded-full px-2.5 py-0.5">
      ...
      <div className="flex flex-col gap-1.5">
        {node.companies.slice(0, 5).map(co => (
          <Link
            ...
            className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl bg-slate-50 hover:bg-gold-50 hover:text-gold-600 group/chip transition-colors"
          >
            <Avatar co={co} />
            <span className="text-xs font-medium text-slate-700 group-hover/chip:text-gold-600 transition-colors truncate">{co.name}</span>
...
export default function NetworkClient({ nodes, allCompanies }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
      ...
      {selectedNode && (
        <div className="mb-5 bg-gold-50 rounded-2xl border border-violet-200 px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold text-gold-500 uppercase tracking-wider mb-0.5">Selected co-investor</p>
            ...
            <button
              onClick={() => setSelectedInvestor(null)}
              className="p-1.5 text-gold-300 hover:text-gold-600 hover:bg-gold-100 rounded-lg transition-colors flex-shrink-0"
            >
              <X size={14} />
            </button>
          </div>
          ...
          {siblingInvestors.size > 0 && (
            <div className="mt-3 pt-3 border-t border-violet-200">
              <p className="text-xs text-gold-500 font-medium mb-2">Co-appears with:</p>
              <div className="flex flex-wrap gap-1.5">
                {[...siblingInvestors].map(inv => (
                  <button
                    key={inv}
                    onClick={() => setSelectedInvestor(inv)}
                    className="text-xs px-2.5 py-1 rounded-full bg-gold-100 text-gold-600 hover:bg-violet-200 transition-colors font-medium"
                  >
                    {inv}
                  </button>
                ))}
          ...
          <div className="flex flex-wrap gap-2">
            {companiesWithout.map(co => (
              <Link
                key={co.id}
                href={`/companies/${co.id}`}
                className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-xl border border-slate-200 text-xs text-slate-500 hover:text-gold-500 hover:border-violet-200 transition-colors"
              >
                <Avatar co={co} />
                {co.name}
              </Link>
