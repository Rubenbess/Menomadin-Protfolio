...
export function ValuationChart({ rounds, ownershipPct }: { rounds: Round[]; ownershipPct?: number }) {
  ...
  const showFMV = ownershipPct != null && ownershipPct > 0
  ...
  return (
    <div className="bg-neutral-50 rounded-xl p-4 mb-4 ring-1 ring-slate-200">
      <p className="text-xs font-semibold text-neutral-600 uppercase tracking-wider mb-3">Valuation Timeline</p>
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={data} margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
          ...
          />
          <Tooltip
            formatter={(v, name) => [`$${(Number(v) / 1_000_000).toFixed(2)}M`, name]}
            contentStyle={{ borderRadius: 10, border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', fontSize: 12 }}
          />
          <Legend iconType="circle" iconSize={7} wrapperStyle={{ fontSize: 11, color: '#64748b' }} />
          ...
          {showFMV && (
            <Line type="monotone" dataKey="Our FMV" stroke="#10b981" strokeWidth={2} dot={{ r: 4, fill: '#10b981' }} connectNulls />
          )}
        </LineChart>
      </ResponsiveContainer>
    </div>
